...
const useSiswaModule = () => {
  ...
  const updateSiswa = async (id: number,
    ...
    return axiosAuthClient
      .put(`/siswa/update/${id}`, payload)
      .then((res) => res.data);
  };
  const useUpdateSiswa = (id: number) => {
    const { mutate, isLoading } = useMutation(
      (payload: SiswaUpdatePayload) => updateSiswa(id, payload),
      {
        onSuccess: (response) => {
          toastSuccess(response.message);
          queryClient.invalidateQueries([`siswa/detail/${id}`]);
  ...
  const useCreateSiswa = () => {
    const { mutate, isLoading } = useMutation(
      async (payload: SiswaCreatePayload) => {
        if (!!payload.file === true) {
          const res = await uploadSingle(payload.file);
          console.log("res", res);
