...
export default function AdminLayout({ children }: { children: ReactNode }) {
  ...
  return (
    ...
    bg-[#16ab39] border-e border-gray-200
    lg:block lg:translate-x-0 lg:end-auto lg:bottom-0"
      >
        <div className="px-8 pt-4 ">
          <a
            className="flex-none rounded-xl text-3xl inline-block font-extrabold focus:outline-none focus:opacity-80 text-white"
            aria-label="MySchool"
          >
            SekolahQu
          </a>
        ...
        >
          <ul className="space-y-5">
            {menus.map((e, i) => {
              const isActive = pathname === `/admin/${e.route}`;
              return (
                <button
                  key={i}
                  className={clsx(
                    `w-full flex items-center justify-between py-3 px-3 bg-white rounded-lg hover:bg-gray-300 text-black text-base  hover:outline transition-all font-bold `,
                    {
                      "bg-white hover:bg-[#16ab39]": !isActive,
                      "bg-red-500 hover:bg-[#16ab39]": isActive,
                    }
                  )}
                  onClick={() => router.push(`/admin/${e.route}`)}
                >
                  {e.label}
                  {e.icon}
                </button>
            ...
            <li>
              <div className="dropdown w-full">
                <div
                  tabIndex={0}
                  role="button"
                  className="w-full flex items-center justify-between py-3 px-3 bg-white rounded-lg hover:bg-gray-300 text-black text-base  hover:outline transition-all font-bold"
                >
                  Guru dan Siswa
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                  ...
                  {role.map((e, i) => {
                    ...
                    return (
                      <button
                        key={i}
                        className={clsx(
                          ` flex items-center justify-between py-3 px-2 bg-white rounded-lg hover:bg-gray-300 text-black text-base  hover:outline transition-all font-bold `,
                          {
                            "bg-white hover:bg-[#16ab39]": !isActive,
                            "bg-red-500 hover:bg-[#16ab39]": isActive,
                          }
